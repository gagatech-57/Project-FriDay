...
export default function WhatsAppResetModal({ initialMode = 'password', onClose, onResetSuccess }) {
  ...
  // Resend Timer (45 Seconds Cooldown)
  ...
  const handleResendOtp = async () => {
    ...
    if (res.success) {
      setSuccessMsg('New WhatsApp Security Code dispatched!');
      if (res.whatsappUrl) setWhatsappUrl(res.whatsappUrl);
      if (res.otpSimulated) setSimulatedOtpNotice(res.otpSimulated);
      setOtpCode('');
      setResendTimer(45);
    } else {
      setErrorMsg(res.message || 'Failed to resend WhatsApp code.');
    }
  };
  ...
  return (
    <div className="passkey-modal-overlay">
      <div className="passkey-card" style={{ maxWidth: '440px' }}>
        ...
        {/* STEP 2 FORM: Verify WhatsApp OTP Code with Resend Timer */}
        {step === 2 && (
          <form onSubmit={handleVerifyOtpCode}>
            ...
            {whatsappUrl && (
              <div style={{ marginBottom: '16px' }}>
                ...
                >
                  ...
                  <span>Open WhatsApp App to View Code</span>
            ...
            {/* Resend WhatsApp Code Cooldown Timer Button */}
            <div style={{ marginTop: '12px', textAlign: 'center' }}>
              <button
                ...
                style={{
                  background: 'none',
                  border: 'none',
                  color: resendTimer > 0 ? 'var(--text-muted)' : '#25d366',
              ...
              >
                {resendTimer > 0 ? (
                  ...
                  <>
                    <RotateCw size={14} color="#25d366" />
                    <span>Resend Code via WhatsApp</span>
                  </>
                )}
              </button>
            </div>
